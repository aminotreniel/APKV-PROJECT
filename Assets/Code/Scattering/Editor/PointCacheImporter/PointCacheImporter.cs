...
using PointCacheAsset = UnityEditor.Experimental.VFX.Utility.PointCacheAsset;
using PCache = UnityEditor.Experimental.VFX.Utility.PCache;

namespace TimeGhost
{
    [ScriptedImporter(1, "pcache2")]
    class PointCacheImporter2 : ScriptedImporter
    {
        ...
        public override void OnImportAsset(AssetImportContext ctx)
        {
            try
            {
                ...
                {
                    bool useHalf = !kvp.Key.Equals("pcPosition");
                    ...
                    switch (kvp.Value.PropertyType)
                    {
                        ...
                        case "float":
                            if (kvp.Value.Size == 1)
                            {
                                surfaceFormat = useHalf ? TextureFormat.RHalf : TextureFormat.RFloat;
                            }
                            else
                            {
                                surfaceFormat = useHalf ? TextureFormat.RGBAHalf : TextureFormat.RGBAFloat;
                            }
                            break;
