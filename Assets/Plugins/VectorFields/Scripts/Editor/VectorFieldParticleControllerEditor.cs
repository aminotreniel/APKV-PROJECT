...
public class VectorFieldParticleControllerEditor : Editor 
{
	...
	SerializedProperty MinimalInfluence;
	...
	void OnEnable()
	{
		...
		MinimalInfluence = serializedObject.FindProperty("MinimalInfluence");
	...
	public override void OnInspectorGUI()
	{
		...
		EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
		EditorGUILayout.PropertyField(MinimalInfluence);
		...
		VectorFieldParticleController editedObj = target as VectorFieldParticleController;
		Vector3 lossyScale = editedObj.transform.lossyScale;

		if (Vector3.SqrMagnitude(lossyScale - Vector3.one) > 0.0001 && editedObj.GetComponent<ParticleSystem>().main.simulationSpace != ParticleSystemSimulationSpace.World)
		{
			// Scale different than 1, may rise some problems.
			EditorGUILayout.HelpBox("The particle system or one of its parents is scaled, this is not recommended when the simulation is not in world space", MessageType.Error);
		}
	}
}